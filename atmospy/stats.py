...
def fleet_precision(data: pd.DataFrame):
    ...
    if data.shape[1] < 3:
        raise ValueError(
            f"You must have at least three columns; you provided {data.shape[1]}."
        )

    # compute the standard deviation
    sum_of_squares = (data.sub(data.mean(axis=1).values, axis=0) ** 2).values.sum()

    stdev = np.sqrt((1 / (data.shape[0] * data.shape[1] - 1)) * sum_of_squares)
