...
def load_dataset(name, cache=True, data_home=None, **kwargs):
    ...
    if name not in available_dataset_names:
        raise ValueError(
            f"{name} is not a valid option. Please choose one of {available_dataset_names}."
        )
...
def check_for_numeric_cols(data, cols):
    ...
    for col in cols:
        if not pd.core.dtypes.common.is_numeric_dtype(data[col]):
            raise TypeError(
                f"Column `{col}` is not numeric. Please convert to a numeric dtype before proceeding."
            )
